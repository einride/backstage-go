...
func TestClient_GetEntityByName(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		client := newTestClient(t, func(w http.ResponseWriter, r *http.Request) {
			...
			assert.Equal(t, "/api/catalog/entities/by-name/foo/bar/baz", r.URL.Path)
			_, _ = w.Write([]byte(system))
		})
		actual, err := client.GetEntityByName(ctx, &GetEntityByNameRequest{
			Kind:      "foo",
			Namespace: "bar",
			Name:      "baz",
		})
	...
	t.Run("fail", func(t *testing.T) {
		...
		entity, err := client.GetEntityByName(ctx, &GetEntityByNameRequest{
			Kind:      "foo",
			Namespace: "bar",
			Name:      "baz",
		})
